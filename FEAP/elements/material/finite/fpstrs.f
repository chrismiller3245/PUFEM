...
c       5. Force plane stress moduli for a(c33,c33).ne.0    21/02/2014
c-----[--.----+----.----+----.-----------------------------------------]
...
c-----[--.----+----.----+----.-----------------------------------------]
      ...
      real*8     sig(*), aa(6,6), g33, dg33,f33, dsig, a3inv, tol,rtol
...
c     Return if either stress or modulus is zero
      ...
      if(aa(c33,c33).eq.0.0d0 .or. (dsig.eq.0.0d0 .and. ict.gt.2)) then
        conv = .true.
        if(aa(c33,c33).ne.0.0d0) go to 100
        return
      endif
...
c     Perform iteration
      ...
      if(debug) then
        write(iow,2000) n,ict,dg33,g33,f33
      endif

      if(abs(dg33).le.tol*abs(f33) .and. ict.gt.3) then
c     if(abs(dg33).le.tol*abs(f33)) then
c     if(abs(dg33).le.tol*abs(f33).and.abs(dg33).le.rtol*abs(g33)) then
        conv = .true.
      else
        conv = .false.
      endif
...
100   if(conv .or. ict.ge.ictmax) then
...
2000  format(' EL=',i8,' ICT =',i4,' DG33=',1p,1e13.5,' G33=',1p,1e13.5,
     &       ' F33=',1p,1e13.5)
